...
async function runAttempt(agent: AgentConfig, task: string, cwd: string, modelName: string | undefined, signal: AbortSignal, ctx: ExtensionContext) {
  ...
  try {
    ...
    return textFromLastAssistantMessage(session.messages);
  } finally {
...
async function runAgent(agent: AgentConfig, task: string, cwd: string, signal: AbortSignal, ctx: ExtensionContext) {
...
export default function subagents(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "subagent",
    label: "Subagent",
    description: "List or run Markdown-defined subagents. Run one task per call; Pi can execute independent calls in parallel.",
    ...
    promptGuidelines: [
      ...
      "Use separate subagent calls for independent parallel tasks; do not run writers in parallel in the same working directory.",
    ],
    ...
    async execute(_id, params, signal, onUpdate, ctx) {
      ...
      onUpdate?.({
        ...
        details: { agent: agent.name, model: agent.model ?? `${ctx.model?.provider}/${ctx.model?.id}` },
      });
      const output = await runAgent(agent, params.task, ctx.cwd, signal, ctx);
